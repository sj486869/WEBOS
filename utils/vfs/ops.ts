...
export function ensureUniqueName(
  ...
  const siblings = listChildren(state, parentId)
...
export function writeFile(state: VfsState, fileId: VfsNodeId, content: string): VfsState {
...
export function renameNode(state: VfsState, id: VfsNodeId, name: string): VfsState {
...
export function moveNode(state: VfsState, id: VfsNodeId, targetFolderId: VfsNodeId): VfsState {
  const node = state.nodes[id];
